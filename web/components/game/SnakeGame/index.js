...
class SnakeGame extends React.Component {
  ...
  async fetchHighScores() {
    const res = await axios.get("snake/show");
    if (res.status === 200) {
      this.setState({ highScores: res.data.data });
    }
  }

  async saveScore() {
    const res = await axios.post("snake/store", {
      score: this.state.score,
    });
    if (res.status === 200) {
      this.setState({
        highScores: res.data.data,
      });
    }
  }
  ...
  gameLoop() {
    let timeoutId = setTimeout(() => {
      if (!this.state.isGameOver) {
        ...
        if (this.state.checkSaveScore) {
          this.saveScore();
          this.setState({ checkSaveScore: false });
        }
      }
    ...
    return (
      ...
      >
        ...
        <div id="Score" style={{ fontSize: this.state.width / 50 }}>
          HIGH-SCORE: {this.state.highScores[0]?.score} 
